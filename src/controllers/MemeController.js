...
export class MemeController {
    ...
    static async create (req, res) {
        ...
        if (result.error) {
            ...
            res.status(201).json({message: 'recived', data: data})
        }
    }
    ...
    static async edit (req, res) {
        ...
        if (jsonData.find(data => data.id === id)) {
            const data = await MemeModel.edit({ id: id, input: result.data })
            res.json({message: 'editado', data: data})
    ...
    static async delete (req, res) {
        const id = parseInt(req.query.id)
        if (jsonData.find(data => data.id === id)) {
            const data = MemeModel.delete({ id: id })
